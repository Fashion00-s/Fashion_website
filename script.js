...
(() => {
  ...
  const SHEET_URL = "https://script.google.com/macros/s/AKfycbzJZ0LJ29TT2HLvuMnGlVwuV16N5Acb3_q7OL-cQWQ34kpIghGrq3mruqzT4OIGmuoo/exec";
  ...
  // Submit order data to Google Sheets via POST JSON
  async function submitToGoogleSheet(formData) {
    try {
      ...
      // no-cors means we can't read the response body — but if no exception is thrown, the request went through
      return true;
    } catch (error) {
